...
def showSideBySideImages(img1, img2, title="", BGR1=True, BGR2=True):
    ...
    img1 = img1 if (BGR1 or (len(img1.shape) <3)) else cv.cvtColor(img1, cv.COLOR_BGR2RGB)
    img2 = img2 if (BGR2 or (len(img2.shape) <3)) else cv.cvtColor(img2, cv.COLOR_BGR2RGB)
...
def showImage(img1, title="" ,BGR1 = True):
    """
